...
public class PathFinder {
	...
	public static boolean findRoute(Client c, int destX, int destY,
		...
		while (tail != tileQueueX.size() && tileQueueX.size() < pathLength) {
			...
			if (curY > 0
					&& via[curX][curY - 1] == 0
					&& (Region.getClipping(curAbsX, curAbsY - 1,
							c.heightLevel % 4) & 0x1280102) == 0) {
				tileQueueX.add(curX);
			...
			if (curX > 0
					&& via[curX - 1][curY] == 0
					&& (Region.getClipping(curAbsX - 1, curAbsY,
							c.heightLevel % 4) & 0x1280108) == 0) {
		...
		int l5;
		for (int j5 = l5 = via[curX][curY]; curX != c.localX()
				|| curY != c.localY(); j5 = via[curX][curY]) {
			if (j5 != l5) {
				l5 = j5;
				tileQueueX.set(tail, curX);
				tileQueueY.set(tail++, curY);
