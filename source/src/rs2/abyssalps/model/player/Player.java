...
import rs2.util.ISAACRandomGen;
...
public abstract class Player {
	...
	public int[] lostItems = new int[28];
	public int[] lostItemsN = new int[28];
	public int[] lostEquipment = new int[14];
	public int[] lostEquipmentN = new int[14];

	private boolean[] gnomeCourseDone = new boolean[6];

	public boolean[] getGnomeCourseDone() {
		return this.gnomeCourseDone;
	}
	...
	public long lastMYSQLConnection;
	...
	private boolean blockWalking = false;

	public void setBlockWalking(boolean blockWalking) {
		this.blockWalking = blockWalking;
	}

	public boolean blockWalking() {
		return blockWalking;
	}
	...
	private int cookAmount, cookItem;

	public int getCookAmount() {
		return this.cookAmount;
	}

	public int getCookItem() {
		return this.cookItem;
	}

	public void setCookAmount(int amount) {
		this.cookAmount = amount;
	}

	public void setCookItem(int item) {
		this.cookItem = item;
	}

	private boolean dicing = false;

	public boolean isDicing() {
		return this.dicing;
	}

	public void setDicingStatus(boolean status) {
		this.dicing = status;
	}

	private int[] pipeData = new int[3];
	...
	public String lastNameKilled = "";

	public ArrayList<String> killedPlayers = new ArrayList<String>();
	public ArrayList<Integer> attackedPlayers = new ArrayList<Integer>();

	public ArrayList<String> playerIPs = new ArrayList<String>();
	...
	private boolean[] skillActive = new boolean[25];

	public boolean[] skillActive() {
		return skillActive;
	}

	public int gfxsToLoop = 0;
	public int lastGfx = 0;
	...
	public boolean initialized = false, disconnected = false,
			...
			checkHeight, usingSupplies = false, lootBag = false;

	public int saveDelay, playerKilled, pkPoints, totalPlayerDamageDealt,
			...
			xInterfaceId = 0, xRemoveId = 0, xRemoveSlot = 0, tzhaarKilled = 0,
			waveId, frozenBy = 0, poisonDamage = 0, venomDamage = 0,
			teleAction = 0, bonusAttack = 0, lastNpcAttacked = 0,
			killCount = 0, pathX, pathY, followingX, followingY, tileIndex,
			teleportToHeight, interfaceAction = -1, wildyTeleport = -1,
			itemToDestroy = -1;

	/**
	 * Slayer variables
	 */
	private int[] slayerData = new int[3];

	public int[] getSlayerData() {
		return this.slayerData;
	}

	/**
	 * Cyclops minigame - defenders
	 */
	private int defenderToDrop;
	private long lastCyclopSubstract;

	public long getLastCyclopSubstract() {
		return lastCyclopSubstract;
	}

	public void setLastCyclopSubstract(long value) {
		this.lastCyclopSubstract = value;
	}

	public int getDefenderToDrop() {
		return this.defenderToDrop;
	}

	public void setDefenderToDrop(int defender) {
		this.defenderToDrop = defender;
	}

	private boolean isCuttingGems = true, usingGemInterface = false;
	private int gemsToCut = 0, gemId;

	public boolean isCuttingGems() {
		return this.isCuttingGems;
	}

	public boolean usingGemInterface() {
		return this.usingGemInterface;
	}

	public int getAmountToCut() {
		return this.gemsToCut;
	}
	...
	public void setGemCutState(boolean state) {
		this.isCuttingGems = state;
	}

	public void setGemInterfaceState(boolean state) {
		this.usingGemInterface = state;
	}

	public void setGemsToCut(int amount) {
		this.gemsToCut = amount;
	}

	public void setGemId(int id) {
		this.gemId = id;
	}
	...
	public void setJailed(boolean value) {
		this.jailed = value;
	}
	...
	private boolean hitpointsLocked = false;

	public boolean isConstitutionLocked() {
		return hitpointsLocked;
	}

	public void setConstitutionState(boolean state) {
		this.hitpointsLocked = state;
	}
	...
	public long lastPlayerMove, lastPoison, lastVenom, lastPoisonSip,
			...
			lastAntiFire, subFoodDelay;
	public boolean canChangeAppearance = false;
	public boolean mageAllowed;
	public byte poisonMask = 0;
	public byte venomMask = 0;

	public final int[] BOWS = { 9185, 11785, 839, 845, 847, 851, 855, 859, 841,
	 ...
	 * Smithing
	 */
	private int amountToSmelt = 0;

	public int getAmountToSmelt() {
		return this.amountToSmelt;
	}

	public void setAmountToSmelt(int amount) {
		this.amountToSmelt = amount;
	}

	private int itemsToSmith = 0;

	public int getItemsToSmithAmount() {
		return this.itemsToSmith;
	}

	public void setItemSmithAmount(int amount) {
		this.itemsToSmith = amount;
	}
	...
	public boolean inGambleZone() {
		return this.absX <= 2102 && this.absY <= 4475 && this.absX >= 2074
				&& this.absY >= 4455;
	}
	...
	public boolean inMulti() {
		...
		if ((absX >= 3136 && absX <= 3327 && absY >= 3519 && absY <= 3607)
				...
				|| (absX >= 1482 && absY <= 3707 && absX <= 1558 && absY >= 3678)
	...
	public InetAddress connectedLocal;
	...
	private int[] lootItems = new int[28];
	private int[] lootItemsN = new int[28];

	public int[] getLootItems() {
		return lootItems;
	}

	public int[] getLootItemAmount() {
		return lootItemsN;
	}
	...
	public Player(int _playerId) {
		...
		playerAppearance[12] = 7; // skin colour
	...
	void destruct() {
		...
		Server.region.remove(this, absX, absY, heightLevel);
		oldX = absX;
		oldY = absY;
		absX = absY = -1;
		mapRegionX = mapRegionY = -1;
	...
	public boolean smallWalkLength() {
		if (Misc.goodDistance(absX, absY,
				(getMapRegionX() * 8 + walkingQueueX[getLastMovementWrite()]),
				(getMapRegionY() * 8 + walkingQueueY[getLastMovementWrite()]),
				2))
			return false;
		return Misc.goodDistance(absX, absY,
				(getMapRegionX() * 8 + walkingQueueX[getLastMovementWrite()]),
				(getMapRegionY() * 8 + walkingQueueY[getLastMovementWrite()]),
				2);
	}
	...
	public int getNextWalkingDirection() {
		...
		Server.region.move(this, oldX, oldY, heightLevel, absX, absY,
				heightLevel);
		return dir;
	}
	...
	public void getNextPlayerMovement() {
		...
		if (teleportToX != -1 && teleportToY != -1) {
			...
			// System.out.println("TRYING: "+absX+
			// " - y: "+absY+" - to: "+teleportToX+" - y: "+teleportToY+" -
			// h1/h2:"+heightLevel+"/"+teleportToHeight);
			if (teleportToX != absX || teleportToY != absY) {
				// System.out.println("SUCCEED: "+absX+
				// " - y: "+absY+" - to: "+teleportToX+" - y: "+teleportToY+" -
				// h1/h2:"+heightLevel+"/"+teleportToHeight);
				Server.region.move(this, absX, absY, heightLevel, teleportToX,
						teleportToY, teleportToHeight);
			}
			...
			((Client) this).getPA().handleEntityFollowing();
	...
	public void setForceMovement(int xx1, int yy1, int xx2, int yy2,
			int speedd1, int speedd2, int directionn) {
		this.x1 = xx1;
		this.y1 = yy1;
		this.x2 = xx2;
		this.y2 = yy2;
