...
public class Friends {
	...
	public void add(long name) {
		...
		getFriends().add(name);
		Client friend = PlayerHandler.getPlayerByLongName(name);
		if (friend != null && !friend.getIgnores().has(client.getNameAsLong())) {
			boolean online = false;
			if (friend.getPrivateChat() == ONLINE) {
				online = true;
			} else if (friend.getPrivateChat() == FRIENDS
					&& friend.getFriends().has(client.getNameAsLong())) {
				online = true;
			}
			...
			if (client.getPrivateChat() == FRIENDS
					&& friend.getFriends().has(client.getNameAsLong())) {
				friend.getPA().sendFriend(client.getNameAsLong(),
						getWorldId(online));
			}
		}
	}
	...
	public void sendPrivateMessage(long name, byte[] packed) {
		...
		if (friend != null) {
			...
			if (send) {
				friend.getPA().sendPM(client.getNameAsLong(),
						(byte) client.getRank(), packed);
			}
		} else {
			send = false;
		}
	...
	public void notifyFriendsOfUpdate() {
		for (Player plr : PlayerHandler.players) {
			...
			Player friend = plr;
			if (((Client) friend).getFriends().has(client.getNameAsLong())) {
				...
				if (client.disconnected) {
					...
					online = false;
				} else if (client.getPrivateChat() == FRIENDS
						&& !client.getFriends().has(friend.getNameAsLong())) {
					online = false;
				}
				((Client) friend).getPA().sendFriend(client.getNameAsLong(),
						getWorldId(online));
			}
		}
	}
	...
	public void sendList() {
		for (long l : getFriends()) {
			boolean online = false;
			Client friend = PlayerHandler.getPlayerByLongName(l);

			if (friend != null
					&& !friend.getIgnores().has(client.getNameAsLong())) {
				if (friend.getPrivateChat() == ONLINE) {
					online = true;
				} else if (friend.getPrivateChat() == FRIENDS
						&& friend.getFriends().has(client.getNameAsLong())) {
