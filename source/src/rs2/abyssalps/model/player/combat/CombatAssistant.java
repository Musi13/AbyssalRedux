...
public class CombatAssistant {
	...
	public void attackNpc(int i) {
		if (NPCHandler.npcs[i] != null) {
			...
			if (NPCHandler.npcs[i].npcType >= 2463
					&& NPCHandler.npcs[i].npcType <= 2468
					&& c.getMinigame() == null) {
				c.sendMessage("You need to pay to kill in here.");
				c.getPA().movePlayer(2845, 3540, 2);
				c.npcIndex = 0;
				return;
			}
			...
			if (SlayerTable.forId(NPCHandler.npcs[i].npcType) != null) {
				if (c.playerLevel[c.playerSlayer] < SlayerTable.forId(
						NPCHandler.npcs[i].npcType).getLevel()) {
					c.npcIndex = 0;
					c.sendMessage("I need a slayer level of "
							+ SlayerTable.forId(NPCHandler.npcs[i].npcType)
									.getLevel() + " to do this.");
					return;
				}
			}
			...
			if (NPCHandler.npcs[i].underAttackBy > 0
				...
				if (NPCHandler.npcs[i].npcType != 5535
						&& NPCHandler.npcs[i].npcType != 494) {
					c.npcIndex = 0;
			...
			if (c.followId2 != i) {
				c.followId2 = i;
				c.getPA().combatFollowNPC(i);
			}
			if (!Region.canAttack(c, NPCHandler.npcs[i])) {
				c.attackTimer++;
			}
			c.followId2 = i;
			c.followId = 0;
			if (c.attackTimer <= 0) {
				...
				c.attackTimer = getAttackDelay(ItemAssistant.getItemName(
				...
				if (ZulrahNPC(i) && !c.usingBow && !c.usingMagic
						&& !usingCrystalBow() && !usingOtherRangeWeapons) {
					resetPlayerAttack();
					return;
				}
				if (armaNpc(i) && !c.usingBow && !c.usingMagic
						&& !usingCrystalBow() && !usingOtherRangeWeapons) {
					resetPlayerAttack();
					return;
				}
				// System.out.println(c.absX + " " + c.absY + " : " + npc.absX +
				// " " + npc.absY + " : " + npc.distanceTo(c));
				// System.out.println(npc.distanceTo(c));
				if ((npc.distanceTo(c) > 4 && (c.usingRangeWeapon
						&& !c.usingBow && !c.usingMagic))

						|| (npc.distanceTo(c) > 2 && (!c.usingRangeWeapon
								&& usingHally() && !c.usingBow && !c.usingMagic))
						|| (npc.distanceTo(c) > getRequiredDistance() && (!c.usingRangeWeapon
								&& !usingHally() && !c.usingBow && !c.usingMagic))
						|| (npc.distanceTo(c) > 10 && (c.usingBow || c.usingMagic))) {
				...
				if (!Region.canAttack(c, NPCHandler.npcs[i]) && c.inGodwars()) {
					c.attackTimer++;
					return;
				}

				if (!Region.canAttack(c, NPCHandler.npcs[i])
						&& NPCHandler.npcs[i].inCorp()) {
					c.attackTimer++;
					return;
				}
				...
				if (!c.usingMagic
						...
						&& BlowPipe.usingBlowPipe(c)
						&& BlowPipe.ammoLeft(c) <= 0) {
					c.sendMessage("Your blowpipe has run out of darts.");
					c.stopMovement();
					c.npcIndex = 0;
					return;
				}
				...
				if (!c.usingMagic && !c.getCombat().usingCorrectArrows()
						&& c.usingBow && !usingCrystalBow()
				...
				if (!c.usingMagic) {
					c.startAnimation(WeaponAnimations.attackAnimation(c));
				} else {
				...
				if (!c.usingBow && !c.usingMagic && !usingOtherRangeWeapons
						&& !BlowPipe.usingBlowPipe(c)) {
					Experience.calculateDamageNPC(c, i, 0, false);
					c.projectileStage = 0;
				}

				if (c.usingBow && !usingOtherRangeWeapons && !c.usingMagic
						&& !BlowPipe.usingBlowPipe(c)) { // range
					// hit
					// delay
					if (c.fightMode == 2)
						c.attackTimer--;
					...
					Experience.calculateDamageNPC(c, i, 1, false);
				}

				if (usingOtherRangeWeapons && !c.usingMagic && !c.usingBow
						&& !BlowPipe.usingBlowPipe(c)) {
					c.rangeItemUsed = c.playerEquipment[c.playerWeapon];
				...
				if (BlowPipe.usingBlowPipe(c) && !c.usingBow
						&& !usingOtherRangeWeapons && !c.usingMagic) {
					c.startAnimation(5061);
					BlowPipe.deleteBlowpipeAmmo(c);
					c.projectileStage = 1;
					c.oldNpcIndex = i;
					if (c.fightMode == 2)
						c.attackTimer--;
				...
				if (c.usingMagic) { // magic hit delay
					...
					Experience.calculateDamageNPC(c, i, 2, false);
					c.spellId = 0;
					if (!c.autocasting)
						c.npcIndex = 0;
				}
			...
			if (NPCHandler.npcs[c.oldNpcIndex] != null) {
				...
				if (usingDbow()) {
					...
					c.getPA().createPlayersProjectile(pX, pY, offX, offY, 50,
							getProjectileSpeed() - 10, getRangeProjectileGFX(),
							41, 31, c.oldNpcIndex + 1, getStartDelay(), slope,
							36);
					c.getPA().createPlayersProjectile(pX, pY, offX, offY, 50,
							getProjectileSpeed() + 10, getRangeProjectileGFX(),
							46, 36, c.oldNpcIndex + 1, getStartDelay(),
							slope + 6, 36);
					return;
				}
				if (c.playerEquipment[3] == 9185
					...
					slope = 5;
					c.getPA().createPlayersProjectile(pX, pY, offX, offY, 50,
							getProjectileSpeed(), getRangeProjectileGFX(), 46,
							36, c.oldNpcIndex + 1, getStartDelay(), slope, 86);
					return;
				}
				if (BlowPipe.usingBlowPipe(c)) {
					slope = 15;
					c.getPA().createPlayersProjectile(pX, pY, offX, offY, 50,
							getProjectileSpeed(),
							BlowPipe.getBlowPipeProjectile(c), 43, 35,
							c.oldNpcIndex + 1, getStartDelay(), slope, 36);
					return;
				}
				if (c.playerEquipment[3] == 861 && c.usingSpecial) {
					c.getPA().createPlayersProjectile(pX, pY, offX, offY, 50,
							getProjectileSpeed(), getRangeProjectileGFX(), 43,
							35, c.oldNpcIndex + 1, getStartDelay(), 10, 36);
					c.getPA().createPlayersProjectile(pX, pY, offX, offY, 50,
							getProjectileSpeed() + 25, getRangeProjectileGFX(),
							43, 35, c.oldNpcIndex + 1, getStartDelay(), 10, 36);
					return;
				}
				slope = 15;
				c.getPA().createPlayersProjectile(pX, pY, offX, offY, 50,
						getProjectileSpeed(), getRangeProjectileGFX(), 43, 35,
						c.oldNpcIndex + 1, getStartDelay(), slope);
			}
		}
	}
	...
	public void attackPlayer(int i) {
		...
		if (PlayerHandler.players[i] != null) {
			...
			if (Boundary.isIn(c, Boundary.DUEL_ARENAS)) {
				if (Boundary.isIn((Client) PlayerHandler.players[i],
						Boundary.DUEL_ARENAS)) {
					...
					if (session instanceof DuelSession) {
						if (!Boundary.isInSameBoundary(c, session.getOther(c),
								Boundary.DUEL_ARENAS)) {
							c.sendMessage("You cannot attack a player if you're not in the same arena.");
							c.getPA()
									.movePlayer(
											session.getArenaBoundary()
													.getMinimumX(),
											session.getArenaBoundary()
													.getMinimumX(), 0);
							c.getCombat().resetPlayerAttack();
			...
			if (c.absX == PlayerHandler.players[i].absX
				...
				c.getPA().setOpponentCoordinates(PlayerHandler.players[i].absX,
			...
			if (c.followId != i) {
				c.followId = i;
				c.getPA().combatFollowPlayer(i);
			}
			if (!Region.canAttack(c, (Client) PlayerHandler.players[i])) {
				c.attackTimer++;
			}
			c.followId = i;
			c.followId2 = 0;
			if (c.attackTimer <= 0) {
				...
				if (Boundary.isIn(c, Boundary.DUEL_ARENAS)) {
					DuelSession session = (DuelSession) Server
							...
							.getMultiplayerSession(c,
									MultiplayerSessionType.DUEL);
					if (!Objects.isNull(session)) {
						if (session.getRules().contains(Rule.NO_RANGE)
								&& (c.usingBow || c.usingRangeWeapon)) {
							c.sendMessage("<Range has been disabled in this duel!");
							c.getCombat().resetPlayerAttack();
							return;
						}
						if (session.getRules().contains(Rule.NO_MELEE)
								&& (!c.usingBow && !c.usingRangeWeapon && !c.usingMagic)) {
						...
						if (session.getRules().contains(Rule.WHIP_AND_DDS)) {
							...
							if (!weaponName.contains("whip")
									&& !weaponName.contains("dragon dagger")
				...
				if (!c.getCombat().usingCorrectArrows() && c.usingBow
						&& !usingCrystalBow()
						&& c.playerEquipment[c.playerWeapon] != 9185
						&& !c.usingMagic) {
					c.sendMessage("You can't use "
				...
				if (c.duelStatus != 5 && !FightPitsMinigame.inGame(c)
				...
				if (c.usingSpecial && !c.usingMagic) {
					if (Boundary.isIn(c, Boundary.DUEL_ARENAS)) {
						...
						if (Objects.nonNull(session)) {
							if (session.getRules().contains(
									Rule.NO_SPECIAL_ATTACK)) {
				...
				if (c.usingBow && !usingOtherRangeWeapons && !c.usingMagic
					...
					Experience.calculateDamagePlayer(c, i, 1, c.doubleHit);
				}

				if (usingOtherRangeWeapons && !BlowPipe.usingBlowPipe(c)) {
					c.rangeItemUsed = c.playerEquipment[c.playerWeapon];
					c.getItems().deleteEquipment();
				...
				if (BlowPipe.usingBlowPipe(c) && !c.usingBow
					...
					if (c.fightMode == 2)
						c.attackTimer--;
					fireProjectilePlayer();
					Experience.calculateDamagePlayer(c, i, 1, false);
				}
				...
				if (c.usingMagic) { // magic hit delay
					...
					if (c.MAGIC_SPELLS[c.spellId][4] > 0) {
						c.getPA().createPlayersProjectile(pX, pY, offX, offY,
								50, 98, c.MAGIC_SPELLS[c.spellId][4],
					...
					c.oldPlayerIndex = i;
					c.oldSpellId = c.spellId;
					Experience.calculateDamagePlayer(c, i, 2, false);
					c.spellId = 0;
	...
	public void appendVengeanceNPC(int npcId, int damage) {
		if (damage <= 0) {
			return;
		}
		...
		if (c.oldPlayerIndex > 0) {
			if (PlayerHandler.players[c.oldPlayerIndex] != null) {
				...
				if (usingDbow()) {
					...
					c.getPA().createPlayersProjectile(pX, pY, offX, offY, 50,
							getProjectileSpeed() - 10, getRangeProjectileGFX(),
							41, 31, -c.oldPlayerIndex - 1, getStartDelay(),
							slope, 36);
					c.getPA().createPlayersProjectile(pX, pY, offX, offY, 50,
							getProjectileSpeed() + 10, getRangeProjectileGFX(),
							46, 36, -c.oldPlayerIndex - 1, getStartDelay(),
							slope + 6, 36);
					return;
				}
				if (BlowPipe.usingBlowPipe(c)) {
					...
					c.getPA().createPlayersProjectile(pX, pY, offX, offY, 50,
							getProjectileSpeed(),
							BlowPipe.getBlowPipeProjectile(c), 43, 35,
							-c.oldPlayerIndex - 1, getStartDelay(), slope, 36);
					return;
				}
				if (ItemDefinitions.forId(c.playerEquipment[3]).getName()
						.endsWith("crossbow")) {
					slope = 5;
					c.getPA().createPlayersProjectile(pX, pY, offX, offY, 50,
							getProjectileSpeed(), getRangeProjectileGFX(), 46,
							36, -c.oldPlayerIndex - 1, getStartDelay(), slope,
							86);
					return;
				}
				if (c.playerEquipment[3] == 861 && c.usingSpecial) {
					c.getPA().createPlayersProjectile(pX, pY, offX, offY, 50,
							getProjectileSpeed(), getRangeProjectileGFX(), 43,
							35, -c.oldPlayerIndex - 1, getStartDelay(), 10, 36);
					c.getPA().createPlayersProjectile(pX, pY, offX, offY, 50,
							getProjectileSpeed() + 25, getRangeProjectileGFX(),
							43, 35, -c.oldPlayerIndex - 1, getStartDelay(), 10,
							36);
					return;
				}
				slope = 15;
				c.getPA().createPlayersProjectile(pX, pY, offX, offY, 50,
						getProjectileSpeed(), getRangeProjectileGFX(), 43, 35,
						-c.oldPlayerIndex - 1, getStartDelay(), slope, 86);
			}
		}
	}
	...
	public void activateSpecial(int weapon, int i) {
		...
		case 11838:
			c.gfx0(1213);
			c.startAnimation(1132);
			if (c.npcIndex > 0) {
				Experience.calculateDamageNPC(c, c.npcIndex, 0, c.doubleHit);
			} else {
				Experience.calculateDamagePlayer(c, i, 0, false);
				Experience.calculateDamagePlayer(c, i, 2, false);
				PlayerHandler.players[i].gfx0(1196);
			}
			break;

		case 12808:
			c.gfx0(1213);
			c.startAnimation(1133);
			c.specDamage = 1.25;
			c.specAccuracy = 1.65;
			if (c.npcIndex > 0) {
				Experience.calculateDamageNPC(c, c.npcIndex, 0, c.doubleHit);
		...
		case 859: // magic long
			...
			c.gfx100(250);
			c.hitDelay = getHitDelay(ItemAssistant.getItemName(
					c.playerEquipment[c.playerWeapon]).toLowerCase());
			c.projectileStage = 1;
			if (c.fightMode == 2)
				c.attackTimer--;
		...
		case 861: // magic short
			...
			if (c.playerIndex > 0) {
				fireProjectilePlayer();
				Experience.calculateDamagePlayer(c, c.playerIndex, 1, true);
			} else if (c.npcIndex > 0) {
				fireProjectileNpc();
				Experience.calculateDamageNPC(c, c.npcIndex, 1, true);
			}
			break;
	...
	public void handlePrayerDrain() {
		...
		if (toRemove > 0) {
			toRemove /= (1 + (0.035 * c.playerBonus[13]));
		}
	...
	public boolean checkReqs() {
		...
		if (c.playerIndex == c.playerId)
			return false;
		if (FightPitsMinigame.inGame(c)
				&& FightPitsMinigame
						.inGame((Client) PlayerHandler.players[c.playerIndex])) {
			return true;
		}
		if (Boundary.isIn(c, Boundary.PVP_ZONE)) {
			if (!PlayerHandler.players[c.playerIndex].inMulti()) {
				if (PlayerHandler.players[c.playerIndex].underAttackBy != c.playerId
					...
					c.sendMessage(PlayerHandler.players[c.playerIndex].playerName
				...
				if (PlayerHandler.players[c.playerIndex].playerId != c.underAttackBy
					...
					c.sendMessage("I'm already in combat.");
					c.stopMovement();
					c.getCombat().resetPlayerAttack();
					return false;
				}
		...
		if (PlayerHandler.players[c.playerIndex].inDuelArena()) {
			if (!Boundary.isIn(c, Boundary.DUEL_ARENAS)) {
				Client other = (Client) PlayerHandler.players[c.playerIndex];
	...
	public int getAttackDelay(String s) {
		...
		// sara sword, 2400ms
			...
			return 3;
		else if (s.contains("knife") || s.endsWith("blowpipe"))
			return 3;
	...
	public int calculateMeleeAttack() {
		int attackLevel = 10 + (c.playerLevel[0] - c
				.getLevelForXP(c.playerXP[0]));
		...
		if (playerBonus > 130)
			...
			if (c.getLevelForXP(c.playerXP[1]) <= 20)
				formula *= 1.65;
			else
				formula *= 1.40;
		}
		...
		if (c.playerEquipment[3] == 11128
				&& (c.playerEquipment[3] == 6528 || c.playerEquipment[3] == 6525)) {
			formula *= 1.20;
		}
		return formula;
	...
	public int calculateMeleeMaxHit() {
		...
		// salve amulet(e) = 1.2
		int strengthLevel = c.playerLevel[2];
		...
		double baseDamage = 1.3 + (effectiveStrengthDamage / 10)
				+ (c.playerBonus[10] / 80)
				+ ((effectiveStrengthDamage * c.playerBonus[10]) / 640);
		...
		if (c.playerEquipment[3] == 6528
				&& c.playerEquipment[c.playerAmulet] == 11128) {
	...
	public int calculateMeleeDefence() {
		int defenceLevel = 5 + (c.playerLevel[1] - c
	...
	public int getRangeStr(int i) {
		...
		int[][] data = { { 877, 10 }, { 9140, 46 }, { 9145, 36 }, { 9141, 64 },
				...
				{ 810, 10 }, { 811, 14 }, { 12926, 60 }, { 11230, 20 },
				{ 864, 3 }, { 863, 4 }, { 865, 7 }, { 866, 10 }, { 867, 14 },
				{ 868, 24 }, { 825, 6 }, { 826, 10 }, { 827, 12 }, { 828, 18 },
				{ 829, 28 }, { 830, 42 }, { 800, 5 }, { 801, 7 }, { 802, 11 },
				{ 803, 16 }, { 804, 23 }, { 805, 36 }, { 9976, 0 },
				{ 9977, 15 }, { 4212, 70 }, { 4214, 70 }, { 4215, 70 },
				{ 4216, 70 }, { 4217, 70 }, { 4218, 70 }, { 4219, 70 },
				{ 4220, 70 }, { 4221, 70 }, { 4222, 70 }, { 4223, 70 },
				{ 6522, 49 }, { 10034, 15 }, { 12926, 107 } };
	...
	public boolean usingCorrectArrows() {
		int data[][] = { { 841, 882, 884 }, { 843, 882, 884, 886 },
				{ 849, 882, 884, 886, 888 }, { 853, 882, 884, 886, 888, 890 },
				{ 857, 882, 884, 886, 888, 890 },
				{ 861, 882, 884, 886, 888, 890, 892 },
				{ 11235, 882, 884, 886, 888, 890, 11212 },
				{ 9185, 9144, 9244, 9243 }, { 11785, 9144, 9244, 9243 },
				{ 4734, 4740 } };
		for (int i = 0; i < data.length; i++) {
			if (c.playerEquipment[3] == data[i][0]) {
				for (int x = 0; x < data[i].length; x++) {
					if (c.playerEquipment[c.playerArrows] == data[i][x]) {
						return true;
					}
				}
			}
	...
	public int getProjectileSpeed() {
		...
		if (c.oldPlayerIndex > 0) {
			c.distanceToPoint(PlayerHandler.players[c.oldPlayerIndex].getX(),
					PlayerHandler.players[c.oldPlayerIndex].getY());
		} else if (c.oldNpcIndex > 0) {
			c.distanceToPoint(NPCHandler.npcs[c.oldNpcIndex].getX(),
					NPCHandler.npcs[c.oldNpcIndex].getY());
		}
		if (c.playerEquipment[3] == 9185 || c.playerEquipment[3] == 11785) {
			return 55;
		}
		if (usingDbow()) {
			int clientSpeed;
	...
	public int mageAtk() {
		int attackLevel = (c.playerLevel[6] - c.getLevelForXP(c.playerXP[6])) * 2;
		...
		else if (c.playerBonus[3] > 30 && c.playerLevel[1] <= 45)
			formula *= 0.7;
		else {
			formula *= 0.01;
			worstGear = true;
	...
	public static boolean contraMagic(Client c) {
		return c.playerEquipment[c.playerChest] == 2503
				|| c.playerEquipment[c.playerChest] == 11828
				|| c.playerEquipment[c.playerChest] == 4736;
	}

	public int mageDef() {
		int defenceLevel = (((c.playerLevel[1] - c.getLevelForXP(c.playerXP[1])) / 2) + ((c.playerLevel[6] - c
				.getLevelForXP(c.playerXP[6])) / 2)) * 2;
	...
	public int getStartDelay() {
		...
		if (c.usingBow || BlowPipe.usingBlowPipe(c)) {
			return 45;
		}
		switch (c.MAGIC_SPELLS[c.spellId][0]) {
		case 1539:
	...
	public void recoilNPC(int damage, int index) {
		if (damage > 0 && c.playerEquipment[c.playerRing] == 2550) {
			int recDamage = damage / 10 + 1;
			if (recDamage > NPCHandler.npcs[index].HP) {
				recDamage = NPCHandler.npcs[index].HP;
			}
			NPCHandler.npcs[index].HP -= recDamage;
			NPCHandler.npcs[index].hitDiff = recDamage;
	...
	public int getBonusAttack(int i) {
		...
		case 319:
			return Experience.usingSpear(c) ? Misc.random(200) + 100 : 0;
			...
			if (c.goodDistance(c.getX(), c.getY(), o.getX(), o.getY(),
				...
				if (checkReqs()) {
					if (checkSpecAmount(4153)) {
						...
						c.specAccuracy = 1.45;
						c.gfx100(340);
						Experience.calculateDamagePlayer(c, c.playerIndex, 0,
								false);
					}
				}
			}
		} else if (c.npcIndex > 0) {
			NPC o = (NPC) NPCHandler.npcs[c.npcIndex];
			if (c.goodDistance(c.getX(), c.getY(), o.getX(), o.getY(),
					getRequiredDistance())) {
				if (checkSpecAmount(4153)) {
					c.startAnimation(1667);
					c.gfx100(340);
					Experience.calculateDamageNPC(c, c.npcIndex, 0, false);
				}
			}
		}
	}

	public void handleGmaulPlayer1() {
		if (c.playerIndex > 0) {
			Client o = (Client) PlayerHandler.players[c.playerIndex];
			if (c.goodDistance(c.getX(), c.getY(), o.getX(), o.getY(),
					getRequiredDistance())) {
				if (checkReqs()) {
					if (checkSpecAmount(12848)) {
						c.startAnimation(1667);
						c.specAccuracy = 1.45;
						c.gfx100(340);
						Experience.calculateDamagePlayer(c, c.playerIndex, 0,
	...
	private boolean ZulrahNPC(int i) {
		switch (NPCHandler.npcs[i].npcType) {
