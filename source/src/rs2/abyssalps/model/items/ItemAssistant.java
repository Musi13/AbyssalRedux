...
import rs2.abyssalps.content.WildernessPoints;
...
public class ItemAssistant {
	...
	public int getTotalCount(int itemID) {
		int count = 0;
		for (int j = 0; j < c.playerItems.length; j++) {
			if (ItemDefinitions.forId(itemID + 1) != null
					&& ItemDefinitions.forId(itemID + 1).isNoted()) {
		...
		for (int index = 0; index < Config.BANK_SIZE; index++) {
			for (int index1 = 0; index1 < c.getBank().getBankTab().length; index1++) {
				if (c.getBank().getBankTab(index1)
						.contains(new BankItem(itemID + 1))) {
					count++;
				}
			}
		}
	...
	public void zulrahDrops() {
		for (int i = 0; i < c.playerItems.length; i++) {
			c.lostItems[i] = c.playerItems[i];
			c.lostItemsN[i] = c.playerItemsN[i];
		}
		for (int i = 0; i < c.playerEquipment.length; i++) {
			c.lostEquipment[i] = c.playerEquipment[i] + 1;
			c.lostEquipmentN[i] = c.playerEquipmentN[i];
		}
		c.sendMessage("You can buy back lost items by talking to Priestess Zul-Gwenwynig");
	}
	...
	public void dropAllItems() {
			...
			if (o != null) {
				if (tradeable(c.playerItems[i] - 1)) {
					Server.itemHandler.createGroundItem(o,
							...
							c.heightLevel, c.playerItemsN[i], c.killerId);
					addToDroppedItems(droppedItems, new GameItem(
							c.playerItems[i] - 1, c.playerItemsN[i]));
				} else {
					Server.itemHandler.createGroundItem(c,
							c.playerItems[i] - 1, c.getX(), c.getY(),
							c.heightLevel, c.playerItemsN[i], c.playerId);
				}
			} else {
				Server.itemHandler.createGroundItem(c, c.playerItems[i] - 1,
						c.getX(), c.getY(), c.heightLevel, c.playerItemsN[i],
						c.playerId);
			}
		}
		for (int e = 0; e < c.playerEquipment.length; e++) {
			...
			if (o != null) {
				if (tradeable(c.playerEquipment[e])) {
					...
					addToDroppedItems(droppedItems, new GameItem(
							c.playerEquipment[e], c.playerEquipmentN[e]));
				} else {
					Server.itemHandler.createGroundItem(c,
							c.playerEquipment[e], c.getX(), c.getY(),
							c.heightLevel, c.playerEquipmentN[e], c.playerId);
				}
			} else {
		...
		for (int index = 0; index < c.getLootItems().length; index++) {
			if (c.getLootItems()[index] <= 0) {
				continue;
			}
			Server.itemHandler.createGroundItem(o, c.getLootItems()[index],
					c.getX(), c.getY(), c.heightLevel,
					c.getLootItemAmount()[index], c.killerId);
		}
		for (int index = 0; index < droppedItems.size(); index++) {
			LogSystem
					.writeToFile(
							c.playerName,
							droppedItems.get(index).amount
									+ " - "
									+ ItemAssistant.getItemName(droppedItems
											.get(index).id), LogTypes.DEATH);
		}
		droppedItems.clear();
		if (o != null) {
			WildernessPoints.givePoints(o, c);
		}
		Server.itemHandler
				.createGroundItem(o == null ? c : o, 526, c.getX(), c.getY(),
						c.heightLevel, 1, o == null ? c.playerId : c.killerId);
	}
	...
	private void addToDroppedItems(List<GameItem> droppedItems, GameItem item) {
		for (GameItem listElement : droppedItems) {
			if (listElement.id == item.id) {
				int newAmount = listElement.amount + item.amount;
				listElement.amount = newAmount;
				return;
			}
		}
	...
	public boolean addItem(int item, int amount) {
		...
		if ((((freeSlots() >= 1) || playerHasItem(item, 1)) && ItemDefinitions
				.forId(item).isStackable())
				|| ((freeSlots() > 0) && !ItemDefinitions.forId(item)
						.isStackable())) {
			for (int i = 0; i < c.playerItems.length; i++) {
				if ((c.playerItems[i] == (item + 1))
						&& ItemDefinitions.forId(item).isStackable()
						&& (c.playerItems[i] > 0)) {
	...
	public final String[] BONUS_NAMES = { "Stab", "Slash", "Crush", "Magic",
			...
			"Range Strength", "Magic Strength", "Prayer" };
		...
		for (int i = 0; i < c.playerEquipment.length; i++) {
			if (c.playerEquipment[i] > -1) {
				for (int k = 0; k < c.playerBonus.length; k++) {
					if (ItemBonuses.forId(c.playerEquipment[i]) == null)
						continue;
					c.playerBonus[k] += ItemBonuses.forId(c.playerEquipment[i])
							.getBonuses()[k];
				}
			}
		}
	...
	public void sendWeapon(int Weapon, String WeaponName) {
		...
		if (WeaponName.equals("Unarmed")) {
			...
			c.getPA().sendFrame126(WeaponName, 3799);
				...
				|| c.playerEquipment[c.playerWeapon] == 10887
	...
	public void getRequirements(String itemName, int itemId) {
		...
		if (itemName.contains("Robin") || itemName.contains("Ranger boots")) {
			c.rangeLevelReq = 40;
			return;
		}
		...
		if (itemName.contains("mithril") && !itemName.endsWith("gloves")) {
			if (!itemName.contains("knife") && !itemName.contains("dart")
					&& !itemName.contains("javelin")
					&& !itemName.contains("thrownaxe")) {
	...
	public static boolean wearItem(Client c, int wearID, int slot) {
		...
		if (c.playerItems[slot] == (wearID + 1)) {
			...
			c.getItems().getRequirements(name, wearID);
			if (wearID == 542 || wearID == 1033 || wearID == 6108) {
				targetSlot = c.playerLegs;
			} else if (wearID == 1035 || wearID == 6107) {
				targetSlot = c.playerChest;
				...
				targetSlot = 3;
					...
					|| name.endsWith("bandana") || name.endsWith("beret")
					|| name.endsWith("cavalier") || name.endsWith(" patch")) {
				targetSlot = c.playerHat;
				...
				targetSlot = c.playerShield;
					...
					|| name.contains("wizard robe") || name.endsWith("shirt")
					|| name.endsWith("tabard")
					|| name.equals("rock-shell plate")) {
				targetSlot = c.playerChest;
			} else if (name.contains("cloak") || name.contains("cape")
					|| name.contains("accumulator")
					|| name.contains("attractor") || name.endsWith("apparatus")) {
				targetSlot = c.playerCape;
			} else if (name.contains("boots") || name.contains("flippers")
					|| name.contains("shoes")) {
				targetSlot = c.playerFeet;
			} else if (name.contains("glove") || name.contains("bracelet")
					|| name.contains("vamb") || name.contains("bracers")) {
				targetSlot = c.playerHands;
			} else if (wearID == 7462) {
				targetSlot = c.playerHands;
			} else if (name.contains("arrow") || name.contains("bolt")) {
				targetSlot = c.playerArrows;
			} else if (name.contains("ring")) {
				targetSlot = c.playerRing;
			} else if (name.contains("amulet") || name.contains("necklace")
					|| name.contains("stole") || name.contains("talisman")) {
				targetSlot = c.playerAmulet;
			} else {
				targetSlot = 3;
			}
			if (Boundary.isIn(c, Boundary.DUEL_ARENAS)) {
				...
				if (!Objects.isNull(session)) {
					...
					if (session.getRules().contains(Rule.NO_SHIELD)) {
						if (targetSlot == c.playerShield
								|| targetSlot == c.playerWeapon
								&& c.getItems().is2handed(
										getItemName(wearID).toLowerCase(),
										wearID)) {
			...
			if (MaxCape.maxItems(wearID)) {
				if (!MaxCape.isMax(c)) {
					c.sendMessage("You need to be max in all skills to wear this item.");
					return false;
				}
			}
			...
			if (slot >= 0 && wearID >= 0) {
				...
				if (toEquip == toRemove + 1
						&& ItemDefinitions.forId(toRemove).isStackable()) {
					c.getItems().deleteItem(toRemove,
							c.getItems().getItemSlot(toRemove), toEquipN);
					c.playerEquipmentN[targetSlot] += toEquipN;
				} else if (targetSlot != 5 && targetSlot != 3) {
					if (c.getItems().playerHasItem(toRemove, 1)
							&& stackable == true) {
					...
					boolean wearing2h = c.getItems().is2handed(
							getItemName(c.playerEquipment[3]).toLowerCase(),
							c.playerEquipment[3]);
					if (wearing2h) {
						toRemove = c.playerEquipment[3];
					...
					boolean is2h = c.getItems().is2handed(
							getItemName(wearID).toLowerCase(), wearID);
					boolean wearingShield = c.playerEquipment[c.playerShield] > 0;
					boolean wearingWeapon = c.playerEquipment[3] > 0;
					if (is2h) {
							...
							if (c.getItems().freeSlots() > 0) {
								if (c.getItems().playerHasItem(toRemove, 1)
									...
									if (toRemove > 0 && toRemoveN > 0)// c.playerEquipment[targetSlot]
										...
										// toEquip
										// - 1;
										c.getItems().addItem(toRemove,
												toRemoveN);// c.playerEquipmentN[targetSlot]
									// =
									// toEquipN;
								} else {
									c.playerItems[slot] = toRemove + 1;
			...
			c.getItems().sendWeapon(c.playerEquipment[3],
					getItemName(c.playerEquipment[3]));
	...
	public void resetBank() {
		...
		for (int i = 0; i < c.getBank().getBankTab().length; i++) {
			...
			if (tab.getTabId() != 0 && tab.size() > 0 && tab.getItem(0) != null) {
				...
				c.getPA().sendFrame34a(58040 + i,
						c.getBank().getBankTab()[i].getItem(0).getId() - 1, 0,
		...
		c.getOutStream().endFrameVarSizeWord();
		c.flushOutStream();
		c.getPA().sendFrame126("" + c.getBank().getCurrentBankTab().size(),
				58061);
		c.getPA().sendFrame126(Integer.toString(tabId), 5292);
		c.getPA()
				.sendFrame126(
						Misc.capitalize(c.playerName.toLowerCase())
								+ "'s Bank.", 58064);
	}
	...
	public void addItemUnderAnyCircumstance(int itemId, int amount) {
		if (!addItem(itemId, amount)) {
			sendItemToAnyTabOrDrop(new BankItem(itemId, amount), c.getX(),
					c.getY());
		}
	}
	...
	public void sendItemToAnyTabOrDrop(BankItem item, int x, int y) {
		...
		if (bankContains(item.getId() - 2)) {
			if (isBankSpaceAvailable(item)) {
				...
				Server.itemHandler.createGroundItem(c, item.getId() - 1, x, y,
						c.heightLevel, item.getAmount(), c.playerId);
			}
		} else {
			sendItemToAnyTab(item.getId() - 1, item.getAmount());
		}
	...
	public boolean addToBank(int itemID, int amount, boolean updateView) {
		...
		Iterator<BankTab> iterator = Arrays.asList(c.getBank().getBankTab())
				.iterator();
			...
			if (t != null && t.size() > 0) {
				...
				while (iterator2.hasNext()) {
					...
					if (i.getId() == item.getId()
							&& !ItemDefinitions.forId(itemID).isNoted()) {
						if (t.getTabId() != tab.getTabId()) {
							tab = t;
						}
					} else {
						if (ItemDefinitions.forId(itemID).isNoted()
								&& i.getId() == item.getId() - 1) {
		...
		if (tab.getItemAmount(item) == Integer.MAX_VALUE) {
			c.sendMessage("Your bank is already holding the maximum amount of "
	...
	public void addItemToBank(int itemId, int amount) {
		...
		if (tab.freeSlots() == 0) {
			...
			Server.itemHandler.createGroundItem(c, itemId, c.absX, c.absY,
					c.heightLevel, amount, c.playerId);
			return;
		}
		long totalAmount = ((long) tab.getItemAmount(item) + (long) item
				.getAmount());
	...
	public int bankItemAmount(int itemId) {
		int total = 0;
		for (BankTab tab : c.getBank().getBankTab()) {
			total += tab.getItemAmount(new BankItem(itemId + 1));
		}
		return total;
	}
	...
	public void removeFromBank(int itemId, int itemAmount, boolean updateView) {
		...
		if (getItemAmount(itemId) == Integer.MAX_VALUE) {
			c.sendMessage("Your inventory is already holding the maximum amount of "
					+ getItemName(itemId).toLowerCase() + " possible.");
			return;
		}
		if (ItemDefinitions.forId(item.getId() - 1).isStackable() || noted) {
	...
	public boolean addEquipmentToBank(int itemID, int slot, int amount,
		...
		while (iterator.hasNext()) {
			...
			if (t != null && t.size() > 0) {
				...
				while (iterator2.hasNext()) {
					BankItem i = iterator2.next();
					if (i.getId() == item.getId() && !Item.itemIsNote[itemID]) {
						if (t.getTabId() != tab.getTabId()) {
							tab = t;
							c.getBank().setCurrentBankTab(tab);
							resetBank();
	...
	public void swapBankItem(int from, int to) {
		...
		c.getBank().getCurrentBankTab()
				.setItem(from, c.getBank().getCurrentBankTab().getItem(to));
	...
	public void moveItems(int from, int to, int moveWindow, boolean insertMode) {
		...
		if (moveWindow == 5382) {
			...
			if (to > 999) {
				...
				if (!insertMode) {
					BankItem item = c.getBank().getCurrentBankTab()
							.getItem(from);
					c.getBank()
							.getCurrentBankTab()
							.setItem(from,
									c.getBank().getCurrentBankTab().getItem(to));
					c.getBank().getCurrentBankTab().setItem(to, item);
				} else {
					int tempFrom = from;
	...
	public void dropArrowNpc() {
		...
		if (Misc.random(10) >= 4) {
			if (Server.itemHandler.itemAmount(c.playerName, c.rangeItemUsed,
					enemyX, enemyY, height) == 0) {
				Server.itemHandler.createGroundItem(c, c.rangeItemUsed, enemyX,
						enemyY, height, 1, c.getId());
			} else if (Server.itemHandler.itemAmount(c.playerName,
					c.rangeItemUsed, enemyX, enemyY, height) != 0) {
				int amount = Server.itemHandler.itemAmount(c.playerName,
						c.rangeItemUsed, enemyX, enemyY, height);
				Server.itemHandler.removeGroundItem(c, c.rangeItemUsed, enemyX,
						enemyY, height, false);
				Server.itemHandler.createGroundItem(c, c.rangeItemUsed, enemyX,
						enemyY, height, amount + 1, c.getId());
			}
		}
	}

	public void dropArrowPlayer(int index) {
		int enemyX = PlayerHandler.players[index].getX();
		int enemyY = PlayerHandler.players[index].getY();
		int height = PlayerHandler.players[index].heightLevel;
		if (c.playerEquipment[c.playerCape] == 10499)
			return;
		if (Misc.random(10) >= 4) {
			if (Server.itemHandler.itemAmount(c.playerName, c.rangeItemUsed,
	...
	public static String getItemName(int ItemID) {
		if (ItemID > 0 && ItemDefinitions.forId(ItemID) != null) {
	...
	public void destroyItemInterface(int itemId) {// Destroy item created by
													// Remco
		c.itemToDestroy = itemId;
		String itemName = ItemDefinitions.forId(c.itemToDestroy).getName();
		String[][] info = {// The info the dialogue gives
		{ "Are you sure you want to destroy this item?", "14174" },
				{ "Yes.", "14175" }, { "No.", "14176" }, { "", "14177" },
				{ "This item is valuable, you will not", "14182" },
				{ "get it back once clicked Yes.", "14183" },
				{ itemName, "14184" } };
		c.getPA().sendFrame34(c.itemToDestroy, 0, 14171, 1);
	...
	public void destroyItem() {
		if (!c.getItems().playerHasItem(c.itemToDestroy)) {
			c.getPA().removeAllWindows();
			return;
		}
		c.getItems().deleteItem(c.itemToDestroy, 1);
		c.getPA().removeAllWindows();
		return;
	}
}