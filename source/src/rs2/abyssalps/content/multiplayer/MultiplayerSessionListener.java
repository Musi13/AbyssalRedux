...
public class MultiplayerSessionListener {
	 ...
	 * 
	 * @param sessionPending
	 *            Session we are trying to add
	 * @return
	 */
	 ...
	 * 
	 * @param player
	 *            the player possibly causing interference
	...
	public boolean containsSessionInconsistancies(Client player) {
		List<MultiplayerSession> playerMultiplayerSessions = sessions
				.stream()
				.filter(session -> session.getStage().getStage() > MultiplayerSessionStage.REQUEST
						&& session.getPlayers().contains(player))
	 ...
	 * 
	 * @param sender
	 *            the player that requested the trade
	 * @param receiver
	 *            the player that received the request
	 * @return a MultiplayerSession object
	 */
	public MultiplayerSession requestAvailable(Client sender, Client receiver,
			MultiplayerSessionType type) {
		for (MultiplayerSession session : sessions) {
			...
			if (session.getPlayers().containsAll(
	...
	public void removeOldRequests(Client player) {
		sessions.removeAll(Arrays
				.asList(sessions
						.stream()
						.filter(session -> session.getPlayers()
								.contains(player)
	 ...
	 * 
	 * @param player
	 *            the player in the trade session
	 ...
	 * 
	 * @param player
	 *            the player in a session
	 */
	public void finish(Client player, MultiplayerSessionFinalizeType type) {
		for (MultiplayerSession session : sessions) {
			if (session.getPlayers().contains(player)) {
				session.finish(type);
				break;
	...
	/**
	 * Determines if the player exists within any of the current trade sessions
	 * that exist.
	 * 
	 * @param player
	 *            The player that we're examining
	 * @return
	 */
	public boolean inSession(Client player, MultiplayerSessionType type) {
		return sessions
				.stream()
				.anyMatch(
	...
	public boolean inAnySession(Client player) {
		return sessions
				.stream()
				.anyMatch(
						session -> session.getPlayers().contains(player)
								&& session.getStage().getStage() > MultiplayerSessionStage.REQUEST);
	}

	/**
	 * Determins if the list of sessions contains this session. If it does not,
	 * then stop.
	 * 
	 * @param session
	 *            the session
