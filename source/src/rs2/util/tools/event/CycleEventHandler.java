...
public class CycleEventHandler {
	...
	public void stopEvents(Object owner, int id) {
		if (id == -1) {
			throw new IllegalStateException(
					"Illegal identification value, -1 is not permitted.");
		}
	...
	public boolean isAlive(Object owner, int eventId) {
		Optional<CycleEventContainer> op = events
				.stream()
				.filter(container -> container.getOwner().equals(owner)
						&& container.getID() == eventId).findFirst();
		return op.isPresent();
	}
	...
	public boolean isAlive(Object owner) {
		Optional<CycleEventContainer> op = events.stream()
				.filter(container -> container.getOwner().equals(owner))
				.findFirst();
		return op.isPresent();
	}

	public static int WOODCUTTING_ID = 1;
	public static int FISHING_ID = 2;
	public static int MINING_ID = 3;
	public static int COOKING_ID = 4;
	public static int FIREMAKING_ID = 5;
	public static int CRAFTING_ID = 6;
	public static int SMITHING_ID = 7;
	public static int SMELTING_ID = 8;
	public static int FLETCHING_ID = 9;
	public static int SLAYER_TELEPORT_ID = 10;
	public static final int CANNON_EVENT_ID = 11;
