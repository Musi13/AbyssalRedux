...
public class Misc {
	...
	public static String findInput(String message) {
		...
		int seperatorIndex = message.indexOf(" ") < message.indexOf("-") ? message
				.indexOf(" ") : message.indexOf("-");
		return message.substring(seperatorIndex + 1);
	}

	public static boolean contains(int search, int... source) {
		for (int n : source) {
			if (search == n) {
				return true;
			}
	...
	public static int getExcept(int exception, int... source) {
		for (int n : source) {
			if (n != exception) {
				return n;
			}
