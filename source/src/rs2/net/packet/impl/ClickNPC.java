	...
	public void processPacket(Client c, int packetType, int packetSize) {
		...
		case ATTACK_NPC:
			...
			if (NPCHandler.npcs[c.npcIndex].MaxHP == 0
					&& NPCHandler.npcs[c.npcIndex].npcType != 5531
					&& NPCHandler.npcs[c.npcIndex].npcType != 496) {
				c.npcIndex = 0;
				break;
			}
			if (NPCHandler.npcs[c.npcIndex] == null) {
			...
			c.stopMovement();
			c.getPA().combatFollowNPC(c.npcIndex);
			...
			if ((c.usingBow || c.autocasting)
					&& npc.distanceTo(c) <= 7
					&& Region.canAttack(c, NPCHandler.npcs[c.npcIndex])) {
				c.stopMovement();
			}
			...
			if (NPCHandler.npcs[c.npcIndex].npcType == 5531
					|| NPCHandler.npcs[c.npcIndex].npcType == 496) {
				if (!c.usingBow && !c.usingMagic) {
					c.sendMessage("I should probably use a different weapon.");
					return;
				}
			}
			...
			if (!c.usingMagic
					...
					&& BlowPipe.usingBlowPipe(c) && BlowPipe.ammoLeft(c) <= 0) {
				c.sendMessage("Your blowpipe has run out of darts.");
				break;
			}

			if (!c.usingMagic && !usingArrows && c.usingBow
